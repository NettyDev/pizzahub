...
import { PlusCircle, MinusCircle, ShoppingCart, Beef, Vegan, Pizza } from "lucide-react";
import {
  pizzaSizes,
  // pizzaSauces,
  // pizzaToppings,
  ...
  PizzaSauce,
  PizzaTopping
} from "@/components/PizzaComposer/types";
...
export default function ComposerPage() {
  ...
  useEffect(() => {
    fetch("/api/composer")
      ...
      .then((data) => [
        setPizzaSauces(data.sauce),
        setPizzaToppings(data.toppings),
        setSelectedSauce(data.sauce[0]),
        setLoading(false)
      ]);
  }, []);

  const [pizzaSauces, setPizzaSauces] = useState<PizzaSauce[]>([]);
  ...
  const handleToppingToggle = (topping: PizzaTopping) => {
    setSelectedToppings((prev) =>
      prev.find((t) => t.id === topping.id) ? prev.filter((t) => t.id !== topping.id) : [...prev, topping]
    );
  };
  ...
  const getCategoryIcon = (category: PizzaTopping["category"]): React.ReactNode => {
    switch (category) {
      case "meat":
        return <Beef className="inline-block h-5 w-5 text-red-600 mr-1.5" />;
      case "cheese":
        return <Pizza className="inline-block h-5 w-5 text-yellow-500 mr-1.5" />;
      case "vegetable":
        return <Vegan className="inline-block h-5 w-5 text-green-600 mr-1.5" />;
    }
  };
  ...
  const PizzaVisualizer = () => {
    ...
    if (selectedSauce && selectedSauce.image) {
      itemsToVisualize.push(selectedSauce);
    }
    itemsToVisualize.push(...selectedToppings.filter((t) => t.image));
    ...
    return (
      <div className="relative w-full aspect-square max-w-[280px] sm:max-w-[320px] md:max-w-[380px] lg:max-w-[420px] xl:max-w-[480px] mx-auto rounded-full overflow-hidden">
        ...
        {sortedVisualItems.map((item) => (
          ...
          >
            <Image src={item.image!} alt={item.name} layout="fill" objectFit="contain" className="animate-fadeIn" />
          </div>
        ))}
      </div>
    );
    ...
    <div className="w-full bg-[url(/pizza-pattern.png)] flex justify-center ">
      <div className="bg-white w-full max-w-7xl shadow-2xl flex flex-col">
        <div className="bg-red-700 text-white text-center py-8 sm:py-10 px-4 sm:px-6 shadow-md">
          <h1 className="font-extrabold text-3xl sm:text-4xl lg:text-5xl tracking-tight">Skomponuj własną pizzę</h1>
          ...
          <>
            <div className="flex-grow flex flex-col lg:flex-row p-4 sm:p-6 md:p-8 gap-6 md:gap-8">
              <div className="lg:w-1/2 xl:w-3/5 flex flex-col items-center justify-start pt-4 lg:pt-15 lg:sticky lg:top-28 lg:h-[calc(100vh-7rem-var(--header-height,80px))]">
                <div className="w-full">{selectedSize && <PizzaVisualizer />}</div>
                <div className="mt-4 text-center lg:hidden">
                  <span className="text-lg text-red-700">Do zapłaty: </span>
                  <span className="text-2xl font-bold text-red-700">
                    {(typeof totalPrice === "number" ? totalPrice : 0) > 0
                      ? formatPrice(typeof totalPrice === "number" ? totalPrice : 0)
                      : formatPrice(0)}
                  </span>
                </div>
              </div>
                        ...
                        >
                          {pizzaSauces.map((sauce) => (
                            <div key={sauce.id}>
                              ...
                              >
                                {sauce.image && (
                                  <Image src={sauce.image} alt={sauce.name} width={36} height={36} className="mb-1" />
                                )}
                                <span className="font-medium text-xs leading-tight mt-1">{sauce.name}</span>
                                {sauce.price > 0 && (
                                  <span className="text-xxs mt-0.5">(+ {formatPrice(sauce.price)})</span>
                                )}
                              </Label>
                            </div>
                          ))}
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-2">
                          <Button
                            variant="outline"
                            size="icon"
                            className="h-9 w-9 rounded-full border-stone-300 hover:border-red-500"
